...
export function GlobeCdn({
  ...
  useEffect(() => {
    ...
    function init() {
      ...
      baseColor: [1, 1, 1],
      markerColor: [0.1, 0.8, 1], // Cyan for dark mode
      glowColor: [0.1, 0.1, 0.2], // Dark glow
      markerElevation: 0.02,
      markers: markers.map((m) => ({ location: m.location, size: 0.012, id: m.id })),
      arcs: arcs.map((a) => ({ from: a.from, to: a.to, id: a.id })),
      arcColor: [0.75, 0.75, 0.8], // Silver color
  ...
  const pyramidFaceStyle = (nth: number): React.CSSProperties => {
    ...
    const colors = ["#001f3f", "#002b5e", "#003366", "#001122"]
    return {
  ...
  return (
    <div className={`relative aspect-square select-none ${className}`}>
      ...
      <canvas
        ref={canvasRef}
        style={{
          width: "100%", height: "100%", cursor: "default", opacity: 0, pointerEvents: "none",
      ...
      {markers.map((m) => (
        ...
        >
          ...
          <span style={{
            ...
            background: "#8b0000", padding: "2px 6px", borderRadius: 3,
            letterSpacing: "0.05em", whiteSpace: "nowrap" as const,
            boxShadow: "0 1px 3px rgba(139,0,0,0.6)",
            fontWeight: "bold",
          }}>{m.region}</span>
        </div>
      ))}
